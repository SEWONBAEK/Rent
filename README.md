...
- **Spring Framework & Java**
  - 대한민국 백엔드 생태계에서 가장 검증되고 안정적인 프레임워크인 Spring과 정적 타입 언어인 Java를 채택했습니다. 
  - 엔터프라이즈 급 서비스의 기본 아키텍처를 학습하고, 중복 예약 방지 로직 및 트랜잭션 관리를 유연하고 안전하게 처리하기 위해 선택했습니다.
  - 인터셉터(Interceptor) 기능을 활용하여 일반/기업/관리자 등 다중 회원 등급에 따른 접근 권한 제어를 효율적으로 커스텀 구현하는 데 적합하다고 판단했습니다.

- **OAuth 2.0 (Kakao / Naver)**
  - 사용자가 별도의 복잡한 회원가입 절차 없이 기존 플랫폼 계정으로 간편하게 서비스를 이용할 수 있도록 편의성을 최우선으로 고려하여 표준 인증 방식인 OAuth 2.0을 연동했습니다.
  - Access Token을 활용한 안전한 사용자 정보 획득 프로세스를 구축하여 보안성과 UX를 동시에 만족시키고자 했습니다.

- **REST API & 외부 결제 연동 (Kakao Pay)**
  - 프론트엔드와 백엔드 간의 데이터를 명확하게 주고받기 위해 RESTful한 API 구조를 설계했습니다.
  - 카카오페이 단건 결제 API와의 통신을 통해 결제 준비(Ready) 및 승인(Approve), 취소에 이르기까지의 실전적인 웹 서비스 결제 아키텍처 흐름을 이해하고 안정적으로 구현하기 위해 도입했습니다.

- **MySQL**
  - 사용자, 숙소, 객실, 예약, 리뷰 등 서로 유기적으로 연결된 데이터의 무결성을 확실히 보장하기 위해 정석적인 RDBMS인 MySQL을 선택했습니다.
  - 객실 중복 예약을 방지하고, 다각도 숙소 검색 키워드 쿼리를 효율적으로 관리 및 조회하기 위해 채택했습니다.
